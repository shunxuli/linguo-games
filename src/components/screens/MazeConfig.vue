...
import { mazeSizes } from '../../engine/maze'

const game = useGameStore()
const { storage } = useGameServices()

const selectedSize = ref<number>(4)
const canStart = computed(() => selectedSize.value > 0)

watch(() => game.currentScreen, (screen) => {
  if (screen === 'maze-config') {
    const s = storage.getMazeSize()
    if (s) selectedSize.value = s
  }
})

function selectSize(s: number) { selectedSize.value = s; storage.setMazeSize(s) }

function start() {
  if (!canStart.value) return
  storage.setMazeSize(selectedSize.value)
  game.navigateTo('maze-game')
}
</script>
