...
import { sortThemes, sortSizes } from '../../engine/sort'

const game = useGameStore()
const { storage } = useGameServices()

type ThemeKey = keyof typeof sortThemes
const selectedTheme = ref<ThemeKey>('size')
...
function selectTheme(t: ThemeKey) { selectedTheme.value = t; storage.setSortTheme(t) }
function selectSize(s: number) { selectedSize.value = s; storage.setSortSize(s) }

function start() {
  if (!canStart.value) return
  storage.setSortTheme(selectedTheme.value)
  storage.setSortSize(selectedSize.value)
  game.navigateTo('sort-game')
}
</script>
