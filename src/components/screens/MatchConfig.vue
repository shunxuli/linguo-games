...
import { matchThemes, matchSizes } from '../../engine/match'
...
function selectTheme(t: ThemeKey) { selectedTheme.value = t; storage.setMatchTheme(t) }
function selectSize(s: number) { selectedSize.value = s; storage.setMatchSize(s) }

function start() {
  if (!canStart.value) return
  storage.setMatchTheme(selectedTheme.value)
  storage.setMatchSize(selectedSize.value)
  game.navigateTo('match-game')
}
</script>
